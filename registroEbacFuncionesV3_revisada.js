...
function editarAlumno(){
    //Saber nombre y apellido a buscar
    var nombre = prompt("Introduce el nombre y apellido del alumno");

    //Recorre todos los registros hasta encontrar el alumno 
    var SeEncontro = false;
    for(var i = 0; i < registro.length; i++){
        
        if(registro[i].nombre === nombre){ //Si hay coincidiencia de alumno:
          
            //Confirma que se encontro
            SeEncontro = true;

            //Preguntar que cambiar
            var menuEditar = prompt("Registro Encontrado \n Que valor quieres editar (Escribe el numero) \n1. Nombre \n2. Edad \n3. Residencia \n4. Programa Inscrito \n5. Correo \n6. Telefono \n7. Seguro Social" );

            //Segun eleccion, pedir nuevo valor y cambiar
            if (menuEditar === "1"){
            ...
            else  { //Si la eleccion es invalida, avisar
                alert("Opcion Invalida");
            }            
        }    
    }

    //Si se encontro y ya termino de cambiar los datos:
    if(SeEncontro){
        //Mostrar alumnos y avisar
        consultarRegistros();
        alert("Registro guardado con exito");

        //Regresar valor para futuros cambios
        SeEncontro = false;
    }
    //Si no se encontro, preguntar si seguir de nuevo o salir:
    else{
        var nuevaBusqueda = prompt("Registro no encontrado quieres buscar de nuevo? oprime S o N para salir");

        //En caso de seguir, volver al menu.
        if(nuevaBusqueda === "S"){
            editarAlumno();
        }
    }
...
//Funcion para eliminar registros opcion 4 del menu
